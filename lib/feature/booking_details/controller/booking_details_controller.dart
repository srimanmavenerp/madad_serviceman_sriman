...
class BookingDetailsController extends GetxController implements GetxService {
  ...
  Future<void> getBookingDetails(
      {required String bookingID, required bool isSubBooking}) async {
    _isLoading = true;
    update();

    Response response = await bookingDetailsRepo.getBookingDetails(
        bookingID: bookingID, isSubBooking: isSubBooking);

    if (response.statusCode == 200) {
      _bookingDetails = BookingDetailsModel.fromJson(response.body);
      // Reset dropdown
      _dropDownValue = _bookingDetails
              ?.bookingContent?.bookingDetailsContent?.bookingStatus ??
          '';

      Get.find<BookingEditController>().getServiceListBasedOnSubcategory(
        subCategoryId: _bookingDetails
                ?.bookingContent?.bookingDetailsContent?.subCategoryId ??
            _bookingDetails?.bookingContent?.bookingDetailsContent?.subBooking
                ?.subCategoryId ??
            "",
      );
  ...
  void changePaymentStatus(String bookingId, String paymentStatus) async {
    Response response =
        await bookingDetailsRepo.changePaymentStatus(bookingId, paymentStatus);
    if (response.statusCode == 200) {
      showCustomSnackBar('successfully_paid'.tr,
          type: ToasterMessageType.success);
    } else {
      showCustomSnackBar(response.body['message']);
    }
  ...
  Future<void> refreshBookingDetails(
      {required String bookingID, required bool isSubBooking}) async {
    try {
      Response response = await bookingDetailsRepo.getBookingDetails(
        bookingID: bookingID,
      ...
      if (response.statusCode == 200) {
        ...
        showCustomSnackBar("Failed to refresh booking details",
            type: ToasterMessageType.error);
      }
    } catch (e) {
      showCustomSnackBar("Error refreshing booking details: $e",
          type: ToasterMessageType.error);
    }
  }
  ...
  void updateServicePageCurrentState(
      BookingDetailsTabControllerState bookingDetaisTabControllerState) {
  ...
  void _hideCanceledOption() {
    if (Get.find<SplashController>()
            ...
            0 ||
        _bookingDetails?.bookingContent?.providerServicemanCanCancelBooking ==
            0) {
